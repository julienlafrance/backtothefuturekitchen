    ...
    if tables_with_dates:
        ...
        try:
            ...
            if not temporal_df.empty:
                ...
                st.info(f"📊 Période analysée: du {temporal_df['date'].min().strftime('%Y-%m-%d')} "
    ...
    try:
        ...
        if not users_df.empty:
            ...
            fig = px.scatter(users_df, 
                           ...
                           labels={'n_items': 'Nombre de recettes', 'n_ratings': 'Nombre d\'évaluations'},
                           opacity=0.6)
            ...
            with col3:
                st.metric("Évaluations moy./utilisateur", f"{users_df['n_ratings'].mean():.1f}")
            with col4:
...
def main():
    ...
    st.markdown("*Analyse complète des données Food.com avec toutes les tables importées*")
    ...
    # Footer
    st.markdown("---")
    st.markdown("*📊 Mangetamain Analytics - Données Food.com | 🔧 PREPROD Environment*")
