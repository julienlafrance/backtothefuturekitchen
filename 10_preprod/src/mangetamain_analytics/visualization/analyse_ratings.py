...
from scipy.stats import kendalltau, spearmanr, linregress, f_oneway, kruskal, mannwhitneyu
...
try:
    ...
    from mangetamain_data_utils.data_utils import load_clean_interactions
except ImportError:
    ...
    def load_clean_interactions(*args, **kwargs):
        """Fonction fallback si import échoue."""
        return pl.DataFrame()
...
def analyse_ratings_distribution():
    ...
    # Axes
    fig.update_yaxes(title_text="Rating moyen", row=1, col=1)
    fig.update_yaxes(title_text="Rating moyen", row=2, col=1, range=[4.65, 4.72])  # Zoom
    ...
    with col2:
        st.metric("IC 95%", f"±{1.96 * std_rating_weighted / np.sqrt(np.sum(weights)):.4f}")
    with col3:
        st.metric("Corr. volume-qualité", f"{vol_qual_weighted:.3f}")
...
def analyse_ratings_seasonality_1():
    ...
    seasonal_stats['season_cat'] = pd.Categorical(seasonal_stats['season'], categories=season_order, ordered=True)
    ...
    with st.expander("Voir les statistiques détaillées"):
        st.dataframe(seasonal_stats[['season', 'mean_rating', 'std_rating', 'count_ratings', 'unique_users', 'unique_recipes']])
...
def analyse_ratings_seasonality_2():
    ...
    values = list(seasonal_ratings["mean_rating"]) + [seasonal_ratings["mean_rating"].iloc[0]]
    ...
    with col3:
        st.metric("Meilleure saison", f"{best_season['season']} ({best_season['mean_rating']:.4f})")
    with col4:
        st.metric("Écart max", f"{best_season['mean_rating'] - worst_season['mean_rating']:.4f}")
